...
contract AavePMTestSetup is Test, HelperFunctions, AavePM {
    ...
    function setUp() external {
        ...
        (aavePM, helperConfig, contractCreator) = deployAavePM.run();
        ...
        // for this contract address to enable exposed local function testing
        vm.startBroadcast(defaultFoundryCaller);
        IAavePM(address(this)).updateContractAddress("tokenSwapsModule", address(new TokenSwapsModule(address(this))));
        IAavePM(address(this)).updateContractAddress(
            "aaveFunctionsModule", address(new AaveFunctionsModule(address(this)))
        );
        IAavePM(address(this)).updateContractAddress(
            "borrowAndWithdrawUSDCModule", address(new BorrowAndWithdrawUSDCModule(address(this)))
        );
        IAavePM(address(this)).updateContractAddress("rebalanceModule", address(new RebalanceModule(address(this))));
        IAavePM(address(this)).updateContractAddress("reinvestModule", address(new ReinvestModule(address(this))));
        vm.stopBroadcast();
